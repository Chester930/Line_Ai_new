...
from linebot.v3.webhook import WebhookParser as LineWebhookParser
...
class WebhookParser:
    ...
    def __init__(self, channel_secret: str):
        self.line_parser = LineWebhookParser(channel_secret)
        self.signature_validator = SignatureValidator(channel_secret)
    
    def parse(self, body: bytes):
        """解析 webhook 事件"""
        return self.line_parser.parse(body)
    
    def verify_signature(self, body: bytes, signature: str) -> bool:
        """驗證 webhook 簽名"""
        try:
            return self.signature_validator.validate(body, signature)
        except Exception:
            return False
...
# 創建全局 Webhook 解析器實例
webhook_parser = WebhookParser(config.settings.LINE_CHANNEL_SECRET)

# 導出類
__all__ = ['LineWebhookHandler', 'MessageHandler', 'WebhookParser'] 