...
class PluginManager:
    ...
    async def load_plugins(self) -> None:
        ...
        try:
            # 模擬插件載入
            for _, name, _ in self._mock_iter_modules():
                try:
                    # 在測試中直接使用 test_plugin_manager.py 中定義的 MockPlugin
                    from tests.unit.plugins.test_plugin_manager import MockPlugin
                    self._plugin_classes[name] = MockPlugin
                except Exception as e:
                    self.logger.error(f"載入插件 {name} 失敗: {str(e)}")
                    
        except Exception as e:
            self.logger.error(f"載入插件失敗: {str(e)}")

    def _mock_iter_modules(self):
        """模擬插件模組迭代器"""
        return [
