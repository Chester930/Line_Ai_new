...
async def test_config_reload(tmp_path):
    """測試重新加載配置"""
    ...
    # 修改文件
    config_file.write_text('{"app_name": "modified_app"}')
...
def test_path_encoder_with_nested_paths():
    ...
    encoded = json.dumps(test_data, cls=EnhancedPathEncoder)
...
def test_path_encoder_edge_cases():
    ...
    encoded = json.dumps(mixed_data, cls=EnhancedPathEncoder)
    decoded = json.loads(encoded)
